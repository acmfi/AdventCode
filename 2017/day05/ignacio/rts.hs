...
type Finish = Bool
type Steps = Int
type State = (Position, [Int], Len, Steps, Finish)

nextState :: State -> State
nextState s@(pos, stack, len, step, True) = s
nextState s@(pos, !stack, len, step, _) -- Strict evaluation
          | pos >= len = (pos, stack, len, step, True)
          | otherwise = (pos + (stack !! pos), (incrementP pos stack), len, step + 1, False)
