defmodule Day2 do
  ...
  def line_to_data(line) do
    [_, min_amount, max_amount, letter, password] =
      Regex.run(~r/(\d+)-(\d+) ([a-z]): ([a-z]+)/, line)

    %{
      min_amount: String.to_integer(min_amount),
      max_amount: String.to_integer(max_amount),
      letter: letter,
      password: password
    }
  end
  ...
  def parse_input(input) do
    input |> String.split("\n", trim: true) |> Enum.map(&line_to_data/1)
  end

  def is_valid_password?(password_data) do
    apparitions =
      password_data.password
      |> String.graphemes()
      |> Enum.count(fn x -> x == password_data.letter end)

    apparitions >= password_data.min_amount and apparitions <= password_data.max_amount
  end

  def solve_1(input) do
    parsed_input = parse_input(input)

    parsed_input |> Enum.map(&is_valid_password?/1) |> Enum.count(fn x -> x end)
  end

  def is_valid_password2?(password_data) do
    first_position =
      password_data.password |> String.graphemes() |> Enum.at(password_data.min_amount - 1)

    second_postition =
      password_data.password |> String.graphemes() |> Enum.at(password_data.max_amount - 1)

    first_present = password_data.letter == first_position
    second_present = password_data.letter == second_postition

    (first_present and not second_present) or (not first_present and second_present)
  end

  def solve_2(input) do
    parsed_input = parse_input(input)

    parsed_input |> Enum.map(&is_valid_password2?/1) |> Enum.count(fn x -> x end)
  end
end
