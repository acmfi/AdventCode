defmodule Day4 do
  ...
  def sample_valid() do
    ...
    iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719123
    """
  end
  ...
  def regex(),
    do:
      ~r/(ecl:(?<ecl>#?[0-9a-z]+))|(pid:(?<pid>#?[0-9a-z]+))|(eyr:(?<eyr>[0-9]+))|(hcl:(?<hcl>#?[0-9a-z]+))|(byr:(?<byr>[0-9]+))|(iyr:(?<iyr>[0-9]+))|(cid:(?<cid>[0-9]+))|(hgt:(?<hgt>[0-9a-z]+))/

  def regex2(),
    do:
      ~r/(ecl:(?<ecl>amb|blu|brn|gry|grn|hzl|oth))|(pid:(?<pid>\d{9})([^\d]|$))|(eyr:(?<eyr>202[0-9]|2030))|(hcl:(?<hcl>#[0-9a-f]{6}))|(byr:(?<byr>19[2-9][0-9]|200[0-2]))|(iyr:(?<iyr>201[0-9]|2020))|(cid:(?<cid>[0-9]+))|(hgt:(?<hgt>1[5-8][0-9]cm|19[0-3]cm|59in|6[0-9]in|7[0-6]in))/

  def is_valid_passport?(passport) do
    passport_keys = passport |> Map.keys()
    result = ["byr", "ecl", "eyr", "hcl", "hgt", "iyr", "pid"] -- passport_keys
    result == []
  end

  def validate_passport(passport_input, solve) do
    regex_names = Regex.names(regex())
    ...
    regex
    |> Regex.scan(passport_input, capture: :all_names)
    |> Enum.map(&Enum.zip(regex_names, &1))
    |> List.flatten()
  ...
  def solve_1(input) do
    input
    |> parse_input()
    |> Enum.map(&validate_passport(&1, :solve1))
  ...
  def solve_2(input) do
    input
    |> parse_input()
    |> Enum.map(&validate_passport(&1, :solve2))
    |> Enum.filter(fn x -> x end)
    |> Enum.count()
  end
end
