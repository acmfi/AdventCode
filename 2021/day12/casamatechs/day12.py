...
def expand_paths(input,visited,path):
    if input == 'end':
    ...
    if input in caves_map.keys():
        ...
        for out in [x for x in caves_map[input] if x not in visited or big_cave_pattern.match(x)]:
            expand_paths(out,deepcopy(visited),path)

def expand_paths_2(input,visited,path,small_visited):
    if input == 'end':
        complete_paths.add(path + '-end')
        return
    if input in caves_map.keys():
        ...
        for out in [x for x in caves_map[input] if x != 'start' and (x not in visited or (x in visited and not small_visited) or big_cave_pattern.match(x))]:
            if not big_cave_pattern.match(out) and out in visited and not small_visited:
                expand_paths_2(out,deepcopy(visited),path,True)
            else:
                expand_paths_2(out,deepcopy(visited),path,small_visited)

complete_paths = set()
expand_paths('start', set(),'')

print(len(complete_paths))

complete_paths = set()
expand_paths_2('start', set(),'',False)
