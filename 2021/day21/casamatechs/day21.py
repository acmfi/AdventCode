...
def check_universe(p1,p2,p1_score,p2_score,movements={},dice_size=3,winning_score=20):
    ...
    ans = (0,0)
    for d1 in range(1,dice_size+1):
        for d2 in range(1,dice_size+1):
            for d3 in range(1,dice_size+1):
                scr = d1+d2+d3
                n_p1_score = p1_score + ((p1 + scr) % 10 if (p1 + scr) % 10 != 0 else 10)
                n_p1 = (p1 + scr) % 10 if (p1 + scr) % 10 != 0 else 10
                w1,w2 = check_universe(p2,n_p1,p2_score,n_p1_score,movements=movements) # Switch positions between Player 1 and 2
