...
garbageCount :: Tree -> Int
garbageCount Empty = 0
garbageCount (Garbage s) = length s
garbageCount (Group s) = sum (map garbageCount s)

-- Parser code. Here be dragons
garbage :: String -> (Maybe Tree, String)
garbage [] = (Nothing, ":garbage:")
garbage ('>':xs) = (Just $ Garbage "", xs)
garbage ('!':_:xs) = case garbage xs of
  (Just (Garbage s), xss) -> (Just $ Garbage s, xss)
  s -> s
